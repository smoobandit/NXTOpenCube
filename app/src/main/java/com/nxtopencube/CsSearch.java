...
public class CsSearch {
    ...
    private CsCoordCube[] nodeUD = new CsCoordCube[21];
    private CsCoordCube[] nodeRL = new CsCoordCube[21];
    private CsCoordCube[] nodeFB = new CsCoordCube[21];

    private CsCoordCube[][] node0 = new CsCoordCube[6][PRE_IDX_MAX];
    ...
    private int verbose;
    private CsCubieCube cc = new CsCubieCube();
    ...
    public CsSearch() {
        for (int i = 0; i < 21; i++) {
            nodeUD[i] = new CsCoordCube();
            nodeRL[i] = new CsCoordCube();
            nodeFB[i] = new CsCoordCube();
        }
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < PRE_IDX_MAX; j++) {
                node0[i][j] = new CsCoordCube();
            }
        }
    }
     ...
     * B-color, in position U3 we have the L color etc. For example, the "super flip" state is represented as
     ...
     * You can also use {@link CsTools.min2phase.Tools#fromScramble(java.lang.String s)} to convert the scramble string to the
    ...
    private void initSearch() {
        ...
        CsCubieCube pc = new CsCubieCube();
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if ((conjMask & 1 << i) == 0) {
                for (int j = 1; j < preIdxMax; j++) {
                    CsCubieCube.CornMult(CsCubieCube.moveCube[CsCubieCube.preMove[j]], cc, pc);
                    CsCubieCube.EdgeMult(CsCubieCube.moveCube[CsCubieCube.preMove[j]], cc, pc);
                    node0[i][j].set(pc);
    ...
    public synchronized static void init() {
        ...
        CsCubieCube.initMove();
        CsCubieCube.initSym();
        CsCubieCube.initFlipSym2Raw();
        CsCubieCube.initTwistSym2Raw();
        CsCubieCube.initPermSym2Raw();

        CsCoordCube.initFlipMove();
        CsCoordCube.initTwistMove();
        CsCoordCube.initUDSliceMoveConj();

        CsCoordCube.initCPermMove();
        CsCoordCube.initEPermMove();
        CsCoordCube.initMPermMoveConj();
        CsCoordCube.initCombMoveConj();

        CsCoordCube.initMEPermPrun();
        CsCoordCube.initMCPermPrun();
        CsCoordCube.initPermCombPrun();

        if (USE_FULL_PRUN || USE_HUGE_PRUN) {
            CsCubieCube.initUDSliceFlipSym2Raw();
            CsCoordCube.initUDSliceFlipMove();
            CsCoordCube.initTwistMoveConj();
            CsCoordCube.initUDSliceFlipTwistPrun();
            if (USE_HUGE_PRUN) {
                CsCoordCube.initHugePrun();
            }
        } else {
            if (USE_TWIST_FLIP_PRUN) {
                CsCoordCube.initTwistFlipPrun();
            }
            CsCoordCube.initSliceTwistPrun();
            CsCoordCube.initSliceFlipPrun();
        }
    ...
    int verify(String facelets) {
        ...
        try {
            String center = new String(
                    new char[] {
                            facelets.charAt(CsUtil.U5),
                            facelets.charAt(CsUtil.R5),
                            facelets.charAt(CsUtil.F5),
                            facelets.charAt(CsUtil.D5),
                            facelets.charAt(CsUtil.L5),
                            facelets.charAt(CsUtil.B5)
                    }
            );
            for (int i = 0; i < 54; i++) {
        ...
        for (length1 = isRec ? length1 : 0; length1 < sol; length1++) {
            ...
            for (urfIdx = isRec ? urfIdx : 0; urfIdx < 6; urfIdx++) {
                ...
                for (preIdx = isRec ? preIdx : 0; preIdx < preIdxMax; preIdx++) {
                    ...
                    if (preIdx != 0) {
                        ssym &= CsCubieCube.moveCubeSym[CsCubieCube.preMove[preIdx]];
                    }
     ...
     *      2: Try Next Axis
     */
    private int phase1(CsCoordCube node, long ssym, int maxl, int lm) {
        ...
        for (long s = ssym; (s >>= 1) != 0; i++) {
            if ((s & 1) == 1) {
                skipMoves |= CsCubieCube.firstMoveSym[i];
            }
        }
        ...
        for (int axis = 0; axis < 18; axis += 3) {
            ...
            for (int power = 0; power < 3; power++) {
                ...
                move[depth1 - maxl] = m;
                int ret = phase1(nodeUD[maxl], ssym & CsCubieCube.moveCubeSym[m], maxl - 1, axis);
                if (ret == 0) {
                    return 0;
                } else if (ret == 2) {
                    break;
    ...
    private String searchopt() {
        ...
        preIdx = 0;
        for (length1 = isRec ? length1 : 0; length1 < sol; length1++) {
            CsCoordCube ud = node0[0 + urfIdx][0];
            CsCoordCube rl = node0[1 + urfIdx][0];
            CsCoordCube fb = node0[2 + urfIdx][0];
     ...
     *      2: Try Next Axis
     */
    private int phase1opt(CsCoordCube ud, CsCoordCube rl, CsCoordCube fb, long ssym, int maxl, int lm) {
        ...
        for (int axis = 0; axis < 18; axis += 3) {
            ...
            for (int power = 0; power < 3; power++) {
                ...
                move[length1 - maxl] = m;
                int ret = phase1opt(nodeUD[maxl], nodeRL[maxl], nodeFB[maxl], ssym & CsCubieCube.moveCubeSym[m], maxl - 1, axis);
                if (ret == 0) {
                    return 0;
                } else if (ret == 2) {
                    break;
    ...
    private int initPhase2() {
        ...
        for (int i = 0; i < depth1; i++) {
            int m = move[i];
            cidx = CsCoordCube.CPermMove[cidx][CsCubieCube.SymMove[csym][m]];
            csym = CsCubieCube.SymMult[cidx & 0xf][csym];
            cidx >>= 4;

            int cx = CsCoordCube.UDSliceMove[mid & 0x1ff][m];
            mid = CsUtil.permMult[mid >> 9][cx >> 9] << 9 | cx & 0x1ff;
        }
        mid >>= 9;
        int prun = CsCoordCube.getPruning(CsCoordCube.MCPermPrun, cidx * 24 + CsCoordCube.MPermConj[mid][csym]);
        if (prun >= maxDep2) {
        ...
        for (int i = 0; i < depth1; i++) {
            ...
            int cx = CsCoordCube.UDSliceMove[u4e & 0x1ff][m];
            u4e = CsUtil.permMult[u4e >> 9][cx >> 9] << 9 | cx & 0x1ff;

            cx = CsCoordCube.UDSliceMove[d4e & 0x1ff][m];
            d4e = CsUtil.permMult[d4e >> 9][cx >> 9] << 9 | cx & 0x1ff;
        }

        int edge = CsCubieCube.MtoEPerm[494 - (u4e & 0x1ff) + (u4e >> 9) * 70 + (d4e >> 9) * 1680];
        int esym = edge & 0xf;
        edge >>= 4;

        prun = Math.max(prun, Math.max(
                CsCoordCube.getPruning(CsCoordCube.MEPermPrun,
                        edge * 24 + CsCoordCube.MPermConj[mid][esym]),
                CsCoordCube.getPruning(CsCoordCube.EPermCCombPrun,
                        edge * 70 + CsCoordCube.CCombConj[CsCubieCube.Perm2Comb[cidx]][CsCubieCube.SymMultInv[esym][csym]])));
        ...
        if (depth1 >= 2 && move[depth1 - 1] / 3 % 3 == move[depth1 - 2] / 3 % 3) {
            lm = CsUtil.std2ud[Math.max(move[depth1 - 1], move[depth1 - 2]) / 3 * 3 + 1];
        } else if (depth1 >= 1) {
            lm = CsUtil.std2ud[move[depth1 - 1] / 3 * 3 + 1];
            if (move[depth1 - 1] > CsUtil.Fx3) {
                lm = -lm;
            }
        }
        ...
        for (depth2 = maxDep2 - 1; depth2 >= prun; depth2--) {
            ...
            if (preIdx != 0) {
                ...
                int axisPre = CsUtil.preMove[preIdx] / 3;
                int axisLast = move[sol - 1] / 3;
                if (axisPre == axisLast) {
                    int pow = (CsUtil.preMove[preIdx] % 3 + move[sol - 1] % 3 + 1) % 4;
                    move[sol - 1] = axisPre * 3 + pow;
                } else if (depth2 > 1
                        && axisPre % 3 == axisLast % 3
                        && move[sol - 2] / 3 == axisPre) {
                    int pow = (CsUtil.preMove[preIdx] % 3 + move[sol - 2] % 3 + 1) % 4;
                    move[sol - 2] = axisPre * 3 + pow;
                } else {
                    move[sol++] = CsUtil.preMove[preIdx];
                }
            }
    ...
    private int phase2(int eidx, int esym, int cidx, int csym, int mid, int maxl, int depth, int lm) {
        ...
        for (int m = 0; m < 10; m++) {
            if (lm < 0 ? (m == -lm) : CsUtil.ckmv2[lm][m]) {
                continue;
            }
            int midx = CsCoordCube.MPermMove[mid][m];
            int cidxx = CsCoordCube.CPermMove[cidx][CsCubieCube.SymMove[csym][CsUtil.ud2std[m]]];
            int csymx = CsCubieCube.SymMult[cidxx & 0xf][csym];
            cidxx >>= 4;
            if (CsCoordCube.getPruning(CsCoordCube.MCPermPrun,
                    cidxx * 24 + CsCoordCube.MPermConj[midx][csymx]) >= maxl) {
                continue;
            }
            int eidxx = CsCoordCube.EPermMove[eidx][CsCubieCube.SymMoveUD[esym][m]];
            int esymx = CsCubieCube.SymMult[eidxx & 0xf][esym];
            eidxx >>= 4;
            if (CsCoordCube.getPruning(CsCoordCube.EPermCCombPrun,
                    eidxx * 70 + CsCoordCube.CCombConj[CsCubieCube.Perm2Comb[cidxx]][CsCubieCube.SymMultInv[esymx][csymx]]) >= maxl) {
                continue;
            }
            if (CsCoordCube.getPruning(CsCoordCube.MEPermPrun,
                    eidxx * 24 + CsCoordCube.MPermConj[midx][esymx]) >= maxl) {
                continue;
            }
            int ret = phase2(eidxx, esymx, cidxx, csymx, midx, maxl - 1, depth + 1, (lm < 0 && m + lm == -5) ? -lm : m);
            if (ret >= 0) {
                move[depth] = CsUtil.ud2std[m];
                return ret;
            }
        }
        return -1;
    ...
    private String solutionToString() {
        ...
        if (urf < 3) {
            for (int s = 0; s < sol; s++) {
                ...
                sb.append(CsUtil.move2str[CsCubieCube.urfMove[urf][move[s]]]).append(' ');
            }
        } else {
