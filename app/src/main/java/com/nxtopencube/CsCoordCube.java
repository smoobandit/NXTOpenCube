...
class CsCoordCube {
    ...
    //full phase1
    static int[][] UDSliceFlipMove = CsSearch.USE_FULL_PRUN ? new int[N_UDSLICEFLIP_SYM][N_MOVES] : null;
    static char[][] TwistMoveF = CsSearch.USE_FULL_PRUN ? new char[N_TWIST][N_MOVES] : null;
    static char[][] TwistConj = CsSearch.USE_FULL_PRUN ? new char[N_TWIST][16] : null;
    ...
    static int[] TwistFlipPrun = CsSearch.USE_TWIST_FLIP_PRUN ? new int[N_FLIP * N_TWIST_SYM / 8] : null;
    ...
    static void initUDSliceMoveConj() {
        ...
        for (int i = 0; i < N_SLICE; i++) {
            ...
            for (int j = 0; j < N_MOVES; j += 3) {
                CsCubieCube.EdgeMult(c, CsCubieCube.moveCube[j], d);
                UDSliceMove[i][j] = (char) d.getUDSlice();
            }
            for (int j = 0; j < 16; j += 2) {
                CsCubieCube.EdgeConjugate(c, CsCubieCube.SymInv[j], d);
        ...
        for (int i = 0; i < N_SLICE; i++) {
            for (int j = 0; j < N_MOVES; j += 3) {
                ...
                for (int k = 1; k < 3; k++) {
                    ...
                    udslice = CsUtil.permMult[udslice >> 9][cx >> 9] << 9 | cx & 0x1ff;
    ...
    static void initFlipMove() {
        ...
        for (int i = 0; i < N_FLIP_SYM; i++) {
            c.setFlip(CsCubieCube.FlipS2R[i]);
    ...
    static void initTwistMove() {
        ...
        for (int i = 0; i < N_TWIST_SYM; i++) {
            c.setTwist(CsCubieCube.TwistS2R[i]);
            for (int j = 0; j < N_MOVES; j++) {
                CsCubieCube.CornMult(c, CsCubieCube.moveCube[j], d);
    ...
    static void initCPermMove() {
        ...
        for (int i = 0; i < N_PERM_SYM; i++) {
            c.setCPerm(CsCubieCube.EPermS2R[i]);
            for (int j = 0; j < N_MOVES; j++) {
                CsCubieCube.CornMult(c, CsCubieCube.moveCube[j], d);
    ...
    static void initEPermMove() {
        ...
        CsCubieCube d = new CsCubieCube();
        for (int i = 0; i < N_PERM_SYM; i++) {
            c.setEPerm(CsCubieCube.EPermS2R[i]);
            for (int j = 0; j < N_MOVES2; j++) {
                CsCubieCube.EdgeMult(c, CsCubieCube.moveCube[CsUtil.ud2std[j]], d);
    ...
    static void initCombMoveConj() {
        ...
        for (int i = 0; i < N_COMB; i++) {
            ...
            for (int j = 0; j < 16; j++) {
                CsCubieCube.CornConjugate(c, CsCubieCube.SymInv[j], d);
    ...
    static void initUDSliceFlipMove() {
        ...
        for (int i = 0; i < N_UDSLICEFLIP_SYM; i++) {
            c.setUDSliceFlip(CsCubieCube.UDSliceFlipS2R[i]);
            int udslice = CsCubieCube.UDSliceFlipS2R[i] >> 11;
            for (int j = 0; j < N_MOVES; j++) {
                CsCubieCube.EdgeMult(c, CsCubieCube.moveCube[j], d);
                ...
                int fsym = flip & 0x7;
                flip >>= 3;
                int udsliceflip = CsCubieCube.FlipSlice2UDSliceFlip[flip * N_SLICE + UDSliceConj[UDSliceMove[udslice][j] & 0x1ff][fsym]];
                UDSliceFlipMove[i][j] = udsliceflip & 0xfffffff0 | CsCubieCube.SymMult[udsliceflip & 0xf][fsym << 1];
            }
        }
    }
    ...
    static void initTwistFlipPrun() {
        ...
        while (done < N_SIZE) {
            ...
            for (int i = 0; i < N_SIZE; i++, val >>= 4) {
                ...
                int twist = i >> 11;
                int flip = CsCubieCube.FlipR2S[i & 0x7ff];
                int fsym = flip & 7;
                flip >>= 3;
                for (int m = 0; m < N_MOVES; m++) {
                    ...
                    int tsymx = twistx & 7;
                    twistx >>= 3;
                    int flipx = FlipMove[flip][CsCubieCube.Sym8Move[m << 3 | fsym]];
                    int fsymx = CsCubieCube.Sym8MultInv[CsCubieCube.Sym8Mult[flipx & 7 | fsym << 3] << 3 | tsymx];
                    flipx >>= 3;
                    int idx = twistx << 11 | CsCubieCube.FlipS2RF[flipx << 3 | fsymx];
                    ...
                    char sym = CsCubieCube.SymStateTwist[twistx];
                    if (sym == 1) {
                        continue;
                    }
                    for (int k = 0; k < 8; k++) {
                        ...
                        int idxx = twistx << 11 | CsCubieCube.FlipS2RF[flipx << 3 | CsCubieCube.Sym8MultInv[fsymx << 3 | k]];
    ...
    static void initRawSymPrun(int[] PrunTable, final int INV_DEPTH,
        ...
        while (done < N_SIZE) {
            ...
            for (int i = 0; i < N_SIZE; i++, val >>= 4) {
                ...
                int sym = i / N_RAW;
                for (int m = 0; m < N_MOVES; m++) {
                    int symx = SymMove[sym][MoveMapSym ? CsUtil.ud2std[m] : m];
                    int rawx = RawConj[RawMove[raw][MoveMapRaw ? CsUtil.ud2std[m] : m] & 0x1ff][symx & SYM_MASK];
                    symx >>= SYM_SHIFT;
                    ...
                    for (int j = 1, symState = SymState[symx]; (symState >>= 1) != 0; j++) {
                        ...
                        int idxx = symx * N_RAW + RawConj[rawx][j ^ (SymSwitch ? CsCubieCube.e2c[j] : 0)];
            ...
            for (int i = 0; i < N_SIZE;) {
                ...
                for (int end = Math.min(i + 16, N_SIZE); i < end; i++, val >>= 2) {
                    ...
                    for (int m = 0; m < N_MOVES; m++) {
                        ...
                        for (int j = 1, symState = CsCubieCube.SymStateUDSliceFlip[symx]; (symState >>= 1) != 0; j++) {
                            if ((symState & 1) != 1) {
                                continue;
                            }
    ...
    static void initSliceTwistPrun() {
        initRawSymPrun(
                ...
                TwistMove, CsCubieCube.SymStateTwist, 0x3
        );
    }
    ...
    static void initSliceFlipPrun() {
        initRawSymPrun(
                UDSliceFlipPrun, 6,
                UDSliceMove, UDSliceConj,
                FlipMove, CsCubieCube.SymStateFlip, 0x3
        );
    }
    ...
    static void initMEPermPrun() {
        initRawSymPrun(
                MEPermPrun, 7,
                MPermMove, MPermConj,
                EPermMove, CsCubieCube.SymStatePerm, 0x4
        );
    }
    ...
    static void initMCPermPrun() {
        initRawSymPrun(
                MCPermPrun, 10,
                MPermMove, MPermConj,
                CPermMove, CsCubieCube.SymStatePerm, 0x34
        );
    }
    ...
    static void initPermCombPrun() {
        initRawSymPrun(
                EPermCCombPrun, 8,
                CCombMove, CCombConj,
                EPermMove, CsCubieCube.SymStatePerm, 0x44
        );
    }
    ...
    CsCoordCube(CsCoordCube cc) {
        set(cc);
    }

    void set(CsCoordCube node) {
    ...
    int getPackedPruning(boolean isPhase1) {
        int prunm3 = 0;
        if (CsSearch.USE_HUGE_PRUN && !isPhase1) {
        ...
        prun = 0;
        CsCoordCube tmp1 = new CsCoordCube();
        CsCoordCube tmp2 = new CsCoordCube();
        tmp1.set(this);
        tmp1.prun = prunm3;
    ...
    void calcPruning(boolean isPhase1) {
        if (CsSearch.USE_FULL_PRUN || CsSearch.USE_HUGE_PRUN) {
            ...
            prun = Math.max(
                    ...
                    CsSearch.USE_TWIST_FLIP_PRUN ? getPruning(TwistFlipPrun,
                            twist << 11 | CsCubieCube.FlipS2RF[flip << 3 | CsCubieCube.Sym8MultInv[fsym << 3 | tsym]]) : 0);
        }
    }

    void set(CsCubieCube cc) {
        if (CsSearch.USE_FULL_PRUN || CsSearch.USE_HUGE_PRUN) {
            twist = cc.getTwist();
     ...
     *      2: Try Next Axis
     */
    int doMovePrun(CsCoordCube cc, int m, boolean isPhase1) {
        if (CsSearch.USE_FULL_PRUN) {
            twist = TwistMoveF[cc.twist][m];
            flip = UDSliceFlipMove[cc.flip][CsCubieCube.SymMove[cc.fsym][m]];
            fsym = CsCubieCube.SymMult[flip & 0xf][cc.fsym];
            flip >>= 4;
            ...
            flip = FlipMove[cc.flip][CsCubieCube.Sym8Move[m << 3 | cc.fsym]];
            fsym = CsCubieCube.Sym8Mult[flip & 7 | cc.fsym << 3];
            flip >>= 3;

            twist = TwistMove[cc.twist][CsCubieCube.Sym8Move[m << 3 | cc.tsym]];
            tsym = CsCubieCube.Sym8Mult[twist & 7 | cc.tsym << 3];
            twist >>= 3;
